...
\"wd\" refers to:"];
	...
	(*Write xfoil command file*)
		...
		(*Individual output filenames*)
			alpha[[ii]]=If[Abs[alpha[[ii]]]<0.00001,0,alpha[[ii]]];
	...
	(*execute xfoil*)
		cmd = ("cd "<>wd<>" && xfoilP4.exe < xfoil.inp > xfoil.out");
		status = Run[cmd];
		If[status!=0,Print[ToString[StringForm["The xfoilP4.exe Exit with ``\n",status]]];Abort[];
		(*,Print["Sucessfully excuting xfoil!"]*)];
		(*clear file*)
		DeleteFile[(wd<>"\\"<>fname<>".inp")];
		DeleteFile[(wd<>"\\"<>"xfoil.out")];
	...
	(*Read polar file*)
		...
		polXtrfTop = ToExpression[StringSplit[pwrt[[8]],{"=","(top)","(bottom)"}][[2]]];
		polXtrfBottom = ToExpression[StringSplit[pwrt[[8]],{"=","(top)","(bottom)"}][[3]]];
		polMach = ToExpression[StringSplit[pwrt[[9]],{"=","Re","e","Ncrit"}][[2]]];
		temp = {ToExpression[StringSplit[pwrt[[9]],{"=","Re","e","Ncrit"}][[4]]],ToExpression[StringSplit[pwrt[[9]],{"=","Re","e","Ncrit"}][[5]]]};
		polRe = temp[[1]]*10^(temp[[2]]);
		polNcrit = ToExpression[StringSplit[pwrt[[9]],{"=","Re","e","Ncrit"}][[7]]];
		polAlpha = ToExpression[StringSplit[pwrt[[13]]][[1]]];
		polCL = ToExpression[StringSplit[pwrt[[13]]][[2]]];
		polCD = ToExpression[StringSplit[pwrt[[13]]][[3]]];
		polCDp = ToExpression[StringSplit[pwrt[[13]]][[4]]];
		polCm = ToExpression[StringSplit[pwrt[[13]]][[5]]];
		polTopXtr = ToExpression[StringSplit[pwrt[[13]]][[6]]];
		polBotXtr = ToExpression[StringSplit[pwrt[[13]]][[7]]];
	(*read the airfoil*)
...
"
	...
	,(*else*)
		shortresult = {};
		For[ii=1,ii<=Length[option],ii++,
			shortresult = Insert[shortresult,results[[option[[ii,1]],option[[ii,2]]]],-1];
		];
		Return[shortresult];
	];
	);
